...
- `search_execute` は `POST /v1/track` から保存され、駅に紐づく学校経由で popularity / area snapshot weight を更新します。request context が空のときは、直近 72 時間の `search_execute` 駅も context resolution の証拠として使います。weight は config で調整します。
