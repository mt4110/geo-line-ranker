...
- 公開後の状態確認に使う read-only post-launch doctor、data quality doctor、operator feedback loop
- 公開 MVP release candidate の証跡を揃える release readiness guide
...
- release candidate の判断は `just release-readiness` で手順を確認し、`just mvp-acceptance` を固定ゲート、`just data-quality-doctor` を補助証跡として扱います。
