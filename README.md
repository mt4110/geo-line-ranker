...
- `context inspect` による context resolver の read-only 確認 CLI
...
- `search_execute` は `POST /v1/track` から保存され、駅に紐づく学校経由で popularity / area snapshot weight を更新します。request context が空のときは、直近 72 時間の `search_execute` 駅も context resolution の証拠として使います。recent search context の confidence は経過時間に応じて 0.88 から下がり、十分に強い user profile context があればそちらを優先します。weight は config で調整します。
...
- `article` は候補種別として予約されていますが、現在は profile pack と ranking config の両方で明示的に拒否します。`supported_content_kinds`、`enabled_content_kinds`、`score_boosts`、`content_kind_max_ratio` に `article` を追加するには、記事 read model、dataset 入力、scoring / diversity tests、fixtures、API の形が変わる場合の OpenAPI / API docs を同じ変更で用意します。
...
```bash
just setup  # .env, PostgreSQL/Redis, migrate, seed, event-csv import
just dev    # worker and API in one terminal
...
```json
{
  ...
  "context": {
    ...
    "evidence_summary": {
      "primary_kind": "request_line",
      "evidence_count": 1,
      "strongest_strength": 0.95,
